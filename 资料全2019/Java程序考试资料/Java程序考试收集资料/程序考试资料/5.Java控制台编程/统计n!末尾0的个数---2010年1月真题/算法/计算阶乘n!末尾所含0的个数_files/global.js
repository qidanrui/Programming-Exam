...
(function(){
    ...
    BdUtil.hi_trackerLink =function(m, url){
       function E(s){ return encodeURIComponent(s) }
       var n = "hilog__"+ (new Date()).getTime();
        var c = window[n] = new Image();
        c.onload=(c.onerror=function(){window[n] = null;});  
        c.src = 'http://hi.baidu.com/sys/statlog/1.gif?m='+E(m)+'&v='+E(url)+'&_t='+Math.random();  //LOG统计地址
        c = null;    
    }

    BdUtil.ns_tracker = (function (){
        //配置产品线
        var pid = 109;  //space
        var type = 2009;    //暂定为普通的统计编号

        function nstracker(m, url){ 
            return function(){
                var n = "nslog__"+ (new Date()).getTime();
                var c = window[n] = new Image();
                c.onload=(c.onerror=function(){window[n] = null;});  
                c.src = 'http://nsclick.baidu.com/v.gif?pid='+pid+'&url='+encodeURIComponent(url)+'&type='+type+'&m='+m+'&_t='+Math.random();  //LOG统计地址
        ...
        return function(mod_id_or_el, mod_name){
			...
			for(var i = 0, j = as.length; i < j; i ++){
				var a = as[i];
				isIE ? a.attachEvent("onclick", nstracker(mn, a)) : a.addEventListener("click",  nstracker(mn, a), false);
			}
		}
    })();

    BdUtil.ns_trackerLink=function(m, url){ 
                var pid = 109;  //space
                var type = 2009;    //暂定为普通的统计编号
                var n = "nslog__"+ (new Date()).getTime();
                var c = window[n] = new Image();
...
BdUtil.relogin = function(){
    ...
    if(window.location.href=='http://hi.baidu.com' || window.location.href=='http://hi.baidu.com/' || window.location.href=='http://hi.baidu.com/index.html' || window.location.href.toLowerCase()=='http://hi.baidu.com/index%2ehtml'){//fixed the bug of /index
...
if(typeof(PSP_relogin)!="function"){
    var PSP_relogin = function(arg){
        if(arg==0){
